...
const purposeOptions = [
  ...
  {
    value: "explore_ideas",
    label: "To explore multiple perspectives or ideas",
  },
  ...
  {
    value: "create_media",
    label: "To create media or visuals for the assignment",
  },
];
...
export const generateStudentCertificate = (data: CertificateData) => {
  ...
  logoImg.onload = () => {
    ...
    doc.text("AI Tool Usage Declaration", 105, topMargin + scaledHeight + 10, {
      align: "center",
    });
    ...
    const aiToolsTableBody = data.aiTools.map((tool) => [
      ...
      Array.isArray(tool.purpose)
        ? tool.purpose
            .map((p) => getLabelFromValue(p, purposeOptions))
            .join("\n")
    ...
    const declaration =
      "I hereby confirm that the above AI tools were used ethically and in compliance with SJSU's academic integrity policy.";
    ...
    doc.setFont("helvetica", "normal");
    if (data.digitalSignature && data.signatureTimestamp) {
      // Add signature with timestamp
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("Digital Signature:", 20, doc.previousAutoTable.finalY + 35);

      doc.setFont("helvetica", "normal");
      doc.setFontSize(11);
      doc.text(data.digitalSignature, 20, doc.previousAutoTable.finalY + 45);

      doc.setFontSize(9);
      doc.setTextColor("#666666");
      doc.text(
        `Digitally signed on ${data.signatureTimestamp}`,
        20,
        doc.previousAutoTable.finalY + 55
      );
      doc.setTextColor("#333333");
    } else {
      // Fallback to manual signature lines if no digital signature
      doc.line(
        20,
    ...
    doc.setFontSize(8);
    doc.text(
      "This is an official SJSU AI Usage Declaration document.",
      105,
      290,
      { align: "center" }
    );
